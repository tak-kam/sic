...
Joining two strings at run time is the other half and arrived separately, with
`+` on `String` and a fuel charge per byte; §5 still argues that the literal was
the half that mattered, and being right about that is why the two were not one
change.
