...
| - | The agent's authority is the program's manifest: translated into its own permissions where those can hold a constraint, routed back through the broker where they cannot, and a hook that fails closed refuses the shell and puts every tool use in the journal | `docs/design/authority.md` |
